...
class Generator():
    ...
    def __init__(self,data_path='preprocessed_data/',
        ...
        if image_features_filename == None:
            ...
            self.image_features_filename = self.data_path + image_features_filename[0]
            self.image_features_filename_scene = self.data_path + image_features_filename[1]
    ...
    def load_image_features(self):
        ...
        self.image_names_to_features_scene = h5py.File(
                                        self.image_features_filename_scene, 'r')
    ...
    def flow(self, mode):
        ...
        images_batch_scene=empty_batch[2]
        targets_batch = empty_batch[3]
        ...
        while True:
            for data_arg, image_name in enumerate(image_names):
                ...
                targets_batch[batch_counter, :, :]  = self.get_one_hot_target(
                                                            one_hot_caption)
                images_batch[batch_counter, :, :],images_batch_scene[batch_counter, :, :]   = self.get_image_features(
                                                            image_name)

                if batch_counter == self.BATCH_SIZE - 1:
                    yield_dictionary = self.wrap_in_dictionary(captions_batch,
                                                                images_batch,images_batch_scene,
                                                                targets_batch)
    ...
    def make_test_input(self,image_name=None):
        ...
        image_features[0, :, :],image_features_scene[0, :, :] = self.get_image_features(image_name)
        return one_hot_caption, image_features,image_features_scene, image_name
    ...
    def make_empty_batch(self):
        ...
        images_batch = np.zeros((self.BATCH_SIZE, self.MAX_TOKEN_LENGTH,
                                    self.IMG_FEATS))
        images_batch_scene = np.zeros((self.BATCH_SIZE, self.MAX_TOKEN_LENGTH,
                                    self.IMG_FEATS))
        targets_batch = np.zeros((self.BATCH_SIZE,self.MAX_TOKEN_LENGTH,
                                    self.VOCABULARY_SIZE))
        return captions_batch, images_batch ,images_batch_scene, targets_batch
    ...
    def get_image_features(self, image_name):
        image_features = self.image_names_to_features[image_name]\
                                            ['image_features'][:]
        image_features_scene = self.image_names_to_features_scene[image_name]\
                                            ['image_features'][:]
        image_input = np.zeros((self.MAX_TOKEN_LENGTH, self.IMG_FEATS))
        image_input_scene = np.zeros((self.MAX_TOKEN_LENGTH, self.IMG_FEATS))
        # print(self.IMG_FEATS)
        # print(image_features.shape)
        image_input[0,:] =  image_features
        image_input_scene[0,:] =  image_features_scene
        return image_input,image_input_scene
    ...
    def wrap_in_dictionary(self,one_hot_caption,
                           image_features,images_scene_features,
                           one_hot_target):

        return [{'text': one_hot_caption,
                'image_object': image_features,'image_scene': images_scene_features},
